...
const SeatChart = ({ occasion, busBooking, provider, setToggle }) => {
  const [seatsTaken, setSeatsTaken] = useState(false)
  const [hasSold, setHasSold] = useState(false)

  const getSeatsTaken = async () => {
    const seatsTaken = await busBooking.getSeatsTaken(occasion.id)
  ...
  const buyHandler = async (_seat) => {
    ...
    const transaction = await busBooking.connect(signer).mint(occasion.id, _seat, { value: occasion.cost })
  ...
  return (
    <div className="occasion">
      <div className="occasion__seating">
        ...
        {seatsTaken && Array(45).fill(1).map((e, i) =>
          <Seat
            i={i}
            step={1}
            columnStart={6}
